...
import { Car, Wrench, ArrowLeftRight, CheckCircle, RefreshCw, XCircle, Calendar, FileSpreadsheet, ArrowLeft, History, Navigation, Info } from 'lucide-react';
...
const VehicleMgmt: React.FC = () => {
  ...
  const exportBookingsCSV = () => {
      const data = bookings.map(b => ({
          ...
          開始時間: new Date(b.start_time).toLocaleString(),
          結束時間: new Date(b.end_time).toLocaleString(),
          用途: b.purpose,
          狀態: b.status,
  ...
  const exportLogsCSV = () => {
      ...
      const data = filteredLogs.map(l => ({
          日期: new Date(l.created_at).toLocaleString(),
      ...
      {activeTab === 'booking' && (
        <>
            ...
            {vehicles.map(car => {
                ...
                return (
                    <div key={car.id} className={`relative overflow-hidden border rounded-2xl shadow-sm transition-all duration-300 group ${selectedVehicle === car.id ? 'ring-2 ring-accent border-transparent' : 'bg-white hover:border-accent hover:shadow-md'}`}>
                                ...
                                {myUpcomingBookings.map(b => (
                                    <tr key={b.id}>
                                        ...
                                        <td className="px-6 py-4 text-sm text-stone-600 whitespace-nowrap">
                                            {new Date(b.start_time).toLocaleString([], {month:'numeric', day:'numeric', hour:'2-digit', minute:'2-digit'})} 
                                ...
                                <tbody className="bg-white divide-y divide-stone-100">
                                    {displayedLogs.length === 0 ? (
                                        ...
                                        <tr key={log.id} className="hover:bg-stone-50">
                                            <td className="px-6 py-4 text-stone-600 whitespace-nowrap">{new Date(log.created_at).toLocaleDateString()}</td>
