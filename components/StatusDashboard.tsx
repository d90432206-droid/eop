...
import { MapPin, Clock, CheckCircle2, AlertCircle, Plane, Coffee, Building2, RefreshCw, Search, Filter, Globe2, Calendar as CalendarIcon, List, ArrowRight, X, Send, Briefcase, ChevronUp, Phone, CloudRain, Utensils, Plus } from 'lucide-react';
...
const StatusDashboard: React.FC = () => {
  ...
  const renderCalendar = () => {
    // ... Calendar implementation same as before ...
  ...
  return (
    <div className="space-y-6 relative pb-24 md:pb-0">
      ...
      {currentEmp && (
          <div className="bg-gradient-to-br from-orange-500 to-amber-600 rounded-2xl shadow-lg p-4 md:p-6 text-white overflow-hidden relative">
              ...
              <div className="relative z-10 flex flex-row items-center justify-between gap-4 md:gap-6">
                  <div className="flex items-center gap-4 w-full md:w-auto">
                      <div className="w-12 h-12 md:w-20 md:h-20 rounded-full border-4 border-white/30 overflow-hidden bg-white/20 flex items-center justify-center text-xl md:text-3xl font-bold shadow-inner shrink-0">
                      ...
                      <div className="flex-1 min-w-0">
                          <h1 className="text-xl md:text-3xl font-bold tracking-tight truncate">{currentEmp.full_name}</h1>
                          <div className="flex items-center gap-2 text-orange-100 mt-1 font-medium text-xs md:text-base truncate">
                              <span>{currentEmp.department}</span>
                              <span className="w-1 h-1 bg-white/50 rounded-full"></span>
                              <span>{currentEmp.job_title}</span>
                          </div>
                          <div className={`mt-2 md:mt-3 inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs md:text-sm font-bold shadow-md bg-white text-orange-700`}>
                  ...
                  <div className="hidden lg:flex flex-col gap-2 w-full md:w-auto">
                      <p className="text-orange-100 text-xs font-bold uppercase tracking-wider mb-1 text-center md:text-left">快速狀態切換</p>
                      <div className="grid grid-cols-4 gap-2">
                          <button onClick={() => handleQuickStatusUpdate(currentEmp.id, 'in_office')} className="flex items-center justify-center gap-2 px-4 py-3 bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white rounded-xl font-bold transition-all border border-white/10">
                              <Building2 size={18} /> 廠內
                          </button>
                          <button onClick={() => handleQuickStatusUpdate(currentEmp.id, 'meeting')} className="flex items-center justify-center gap-2 px-4 py-3 bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white rounded-xl font-bold transition-all border border-white/10">
                              <CheckCircle2 size={18} /> 會議
                          </button>
                          <button onClick={() => handleQuickStatusUpdate(currentEmp.id, 'out')} className="flex items-center justify-center gap-2 px-4 py-3 bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white rounded-xl font-bold transition-all border border-white/10">
                              <MapPin size={18} /> 外出
                          </button>
                          <button onClick={() => handleQuickStatusUpdate(currentEmp.id, 'abroad')} className="flex items-center justify-center gap-2 px-4 py-3 bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white rounded-xl font-bold transition-all border border-white/10">
                              <Plane size={18} /> 出國
                          </button>
                      </div>
                  </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-stone-200 pb-5 pt-4">
        <div>
            <h2 className="text-xl font-bold text-primary tracking-tight">全廠人員動態</h2>
        ...
        <div className="flex flex-wrap gap-3 items-center w-full md:w-auto">
             <div className="bg-stone-100 p-1 rounded-xl flex border border-stone-200">
                <button onClick={() => setViewMode('list')} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1 ${viewMode === 'list' ? 'bg-white shadow-sm text-stone-800' : 'text-stone-500 hover:text-stone-700'}`}>
                    <List size={14}/> 列表
                </button>
                <button onClick={() => setViewMode('calendar')} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1 ${viewMode === 'calendar' ? 'bg-white shadow-sm text-stone-800' : 'text-stone-500 hover:text-stone-700'}`}>
            ...
            <div className="flex gap-2 flex-1 md:flex-none w-full md:w-auto">
                 <div className="relative flex-1">
                    ...
                    <input type="text" placeholder="搜尋..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-9 pr-4 py-2 border border-stone-300 rounded-xl text-sm focus:ring-2 focus:ring-accent/50 outline-none" />
                </div>
                <div className="relative w-28 md:w-32">
                    <Filter size={16} className="absolute left-3 top-2.5 text-stone-400" />
                    <select value={deptFilter} onChange={e => setDeptFilter(e.target.value)} className="w-full pl-9 pr-2 py-2 border border-stone-300 rounded-xl text-sm focus:ring-2 focus:ring-accent/50 bg-white outline-none appearance-none">
                        {departments.map(d => <option key={d} value={d}>{d === 'All' ? '部門' : d}</option>)}
                    </select>
                </div>
            </div>
        </div>
      ...
      {viewMode === 'list' && abroadEmployees.length > 0 && (
        <div className="bg-sky-50 border border-sky-200 rounded-2xl p-6 mb-6">
            <h3 className="text-sky-800 font-bold flex items-center gap-2 mb-4">
                <Globe2 size={24} /> 目前出國人員 ({abroadEmployees.length})
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {abroadEmployees.map(emp => (
                    <div key={emp.id} className="bg-white p-4 rounded-xl shadow-sm flex flex-col gap-3 border border-sky-100 group relative">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-stone-200 overflow-hidden">
            ...
            <div className="hidden lg:block overflow-x-auto">
                <table className="min-w-full divide-y divide-stone-200">
                    <thead className="bg-stone-50">
                        <tr>
                            <th className="px-6 py-4 text-left text-xs font-bold text-stone-500 uppercase tracking-wider">員工資訊</th>
                            <th className="px-6 py-4 text-left text-xs font-bold text-stone-500 uppercase tracking-wider">部門/職稱</th>
                            <th className="px-6 py-4 text-left text-xs font-bold text-stone-500 uppercase tracking-wider">目前狀態</th>
                            <th className="px-6 py-4 text-left text-xs font-bold text-stone-500 uppercase tracking-wider">備註與時間</th>
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div className="text-sm font-bold text-stone-900 flex items-center gap-2">
                                                    {emp.full_name}
                                                    {emp.phone && <a href={`tel:${emp.phone}`} className="text-stone-300 hover:text-accent"><Phone size={12}/></a>}
                                                </div>
                                                <div className="text-xs text-stone-500">{emp.employee_id}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="text-sm text-stone-900">{emp.department}</div>
                                        <div className="text-xs text-stone-500">{emp.job_title}</div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex gap-2">
                                            ...
                                            {isLunch && <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 text-amber-700 border border-amber-200"><Utensils size={10} className="mr-1"/> 午休</span>}
                                            {showRain && <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-sky-100 text-sky-700 border border-sky-200 animate-pulse"><CloudRain size={10} className="mr-1"/> 雨天</span>}
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="text-sm text-stone-600 space-y-1">
                                            {emp.location_detail && ( <div className="flex items-center gap-1.5 font-medium max-w-xs truncate"><Briefcase size={12} /> <span dangerouslySetInnerHTML={{ __html: emp.location_detail.replace(/(https?:\/\/[^\s]+)/g, '<a href="$1" target="_blank" class="text-accent hover:underline">地圖</a>') }}></span></div> )}
                                            {emp.expected_return && ( <div className="flex items-center gap-1.5 text-stone-500"><Clock size={12} /> {new Date(emp.expected_return).toLocaleString([], {month:'numeric', day:'numeric', hour:'2-digit', minute:'2-digit', hour12: false})}</div> )}
                                            {!emp.location_detail && !emp.expected_return && <span className="text-stone-300">-</span>}
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button onClick={() => handleQuickStatusUpdate(emp.id, 'in_office')} className="text-emerald-600 hover:text-emerald-900 bg-emerald-50 px-2 py-1 rounded text-xs border border-emerald-100 font-bold">廠內</button>
                                            <button onClick={() => handleQuickStatusUpdate(emp.id, 'meeting')} className="text-amber-600 hover:text-amber-900 bg-amber-50 px-2 py-1 rounded text-xs border border-amber-100 font-bold">會議</button>
                                            <button onClick={() => handleQuickStatusUpdate(emp.id, 'out')} className="text-stone-600 hover:text-stone-900 bg-stone-100 px-2 py-1 rounded text-xs border border-stone-200 font-bold">外出</button>
                                            <button onClick={() => handleQuickStatusUpdate(emp.id, 'abroad')} className="text-sky-600 hover:text-sky-900 bg-sky-50 px-2 py-1 rounded text-xs border border-sky-100 font-bold">出國</button>
                                        </div>
                                    </td>
                                </tr>
                            )
            ...
            <div className="lg:hidden">
                {normalEmployees.map(emp => {
                    ...
                    return (
                        <div key={emp.id} className="p-4 border-b border-stone-100 flex items-center justify-between relative bg-white first:rounded-t-2xl last:rounded-b-2xl">
                            <div className="flex items-center gap-3 overflow-hidden">
                                ...
                                <div className="min-w-0">
                                    <div className="font-bold text-stone-800 truncate">{emp.full_name}</div>
                                    ...
                                    <div className="flex gap-1 flex-wrap">
                                        ...
                                        {isLunch && <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 text-amber-700 border border-amber-200"><Utensils size={10} className="mr-1"/> 午休</span>}
                                        {showRain && <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-sky-100 text-sky-700 border border-sky-200"><CloudRain size={10} className="mr-1"/> 雨天</span>}
                                    </div>
                                </div>
                            </div>
                            <div className="text-right flex flex-col items-end gap-1.5 ml-2">
                                ...
                                {emp.phone && (
                                    <a href={`tel:${emp.phone}`} className="mt-1 p-2 bg-stone-50 text-stone-400 hover:text-green-500 hover:bg-green-50 rounded-full border border-stone-200 transition-colors">
                                        <Phone size={14} />
                                    </a>
                                )}
                            </div>
