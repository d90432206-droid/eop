...
const LeaveRequestPage: React.FC = () => {
  ...
  const calculateDurationLogic = (start: Date, end: Date, isOvertimeCalc: boolean = false) => {
      ...
      // Check if it's a weekday for the purpose of the 18-22 rule
      ...
      // 標準工時邏輯：適用於「一般請假」或「假日加班」
      // 規則：08:00~17:30，扣除休息時間
      // 上午工作: 08:00-10:00 (2h)
      // 上午休息: 10:00-10:15 (不計)
      // 上午工作: 10:15-12:15 (2h)
      // 午休時間: 12:15-13:15 (不計)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isOvertime && leaveType === 'business' && transportMode === 'company_car' && !selectedVehicleId) {
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {['18:30', '19:00', '19:30', '20:00', '20:30', '21:00', '21:30', '22:00'].map(t => (
                                        <button 
                                            ...
                                            className={`text-xs px-2 py-1.5 rounded-lg border font-mono font-bold transition-all ${
                                                startTime === '18:00' && endTime === t 
                    ...
                    <div className={`bg-sky-50 p-3 rounded-xl border ${vehicleError ? 'border-red-300' : 'border-sky-100'} space-y-3`}>
                        ...
                        {transportMode === 'company_car' && (
                            <div className="space-y-2 mt-2">
                                <div className="text-xs text-sky-700 font-bold flex items-center gap-1"><Car size={12}/> 選擇車輛 (審核通過後生效)</div>
                                ...
                                <div className="grid grid-cols-1 gap-2 max-h-40 overflow-y-auto custom-scrollbar">
                                    {availableVehicles.map(v => (
                                        <label key={v.id} className={`flex items-center justify-between p-2.5 border rounded-lg cursor-pointer text-xs transition-colors ${selectedVehicleId === v.id ? 'bg-sky-100 border-sky-300 ring-1 ring-sky-300' : 'bg-white border-stone-200 hover:border-sky-200'}`}>
                                            <div className="flex items-center gap-2">
                                                <input type="radio" name="vehicle" className="text-sky-600 focus:ring-sky-500" checked={selectedVehicleId === v.id} onChange={() => {setSelectedVehicleId(v.id); setVehicleError(null);}} />
                                                <span className="font-bold text-stone-700">{v.name}</span>
                                            </div>
                                            <span className="text-stone-400 font-mono">{v.plate_number}</span>
                                        </label>
                                    ))}
                                </div>
                            </div>
